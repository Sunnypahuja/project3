...
//change pass
    function change_pass($current,$new,$confirm,$conn)
    {
        $email=$_SESSION['signed_in'];
...
function calculate_commitions($p_id,$conn)
{
  ...
  //UPDATE FUNCTION FOR ANY DATABASE CHANGES DEPENDS ON LAST UPDATED COLUMN OF USER PLANS
  function update_the_code($conn,$conn_admin)
  {
        $last_updated=$_SESSION['update'];
        // $last_updated="2019-08-03";
      
        //UPDATION DEPENDENCIES
        if($last_updated < "2019-08-04")
        {
            $sql="ALTER TABLE orders ADD description TEXT NULL AFTER shipping_charge";
            $conn->query($sql);
        }
        if($last_updated < "2019-08-04")
        {
            $sql="ALTER TABLE products CHANGE price price BIGINT(50) NOT NULL";
            $conn->query($sql);
        }
        if($last_updated < "2019-08-04")
         {
             $sql="DELETE FROM dynamic_pages WHERE page_name = 'careers'";
             $conn->query($sql);
             $sql="update dynamic_pages set page_name='shipping-and-returns' WHERE page_name = 'shipping-rates-and-policies'";
             $conn->query($sql);
             $sql="update dynamic_pages set page_name='privacy-policy' WHERE page_name = 'taxes'";
             $conn->query($sql);
             $sql="insert into dynamic_pages(page_name,page_data) values('terms-and-conditions','Soon Will Be Updated!')";
             $conn->query($sql);
         }
         if($last_updated < "2019-08-11")
         {
             $sql="ALTER TABLE menus ADD sort_order INT NOT NULL AFTER parent";
             $conn->query($sql);
             $sql="update menus set sort_order=65535";
             $conn->query($sql);
         }
         
         if( $last_updated < "2019-08-22")
         {
             $sql = "CREATE TABLE IF NOT EXISTS png_navbar (id int(11) NOT NULL PRIMARY KEY AUTO_INCREMENT, nav_id int(11) NOT NULL, image_path text NOT NULL, sort_order int(11) NOT NULL)";
             $conn->query($sql);
         }
         
         if($last_updated < "2019-09-08")
         {
             $sql="ALTER TABLE shop_details ADD local_shipping INT NOT NULL AFTER gstin, ADD shipping_scope VARCHAR(50) NOT NULL AFTER local_shipping, ADD max_local_distance DOUBLE NOT NULL DEFAULT '5' AFTER shipping_scope, ADD state VARCHAR(50) NOT NULL AFTER max_local_distance, ADD city VARCHAR(100) NOT NULL AFTER state, ADD pin VARCHAR(10) NOT NULL AFTER city";
             $conn->query($sql);
             
             $sql="CREATE TABLE local_delivery (id int(11) NOT NULL PRIMARY KEY,max_charge float NOT NULL,charge float NOT NULL,distance float NOT NULL)";
             $conn->query($sql);
             
             $sql="ALTER TABLE local_delivery MODIFY id int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6";
             $conn->query($sql);
         }
         
         if($last_updated < "2019-09-15")
         {
             $sql = "create table theme_options(id bigint(50) NOT NULL PRIMARY KEY AUTO_INCREMENT, theme_option text NOT NULL,";
             $sql.="color text NOT NULL, font_color text)";
             $conn->query($sql);
         }
         
         if($last_updated < "2019-10-19")
         {
             $sql = "create table social_links(id bigint(50) NOT NULL PRIMARY KEY AUTO_INCREMENT, media text NOT NULL,";
             $sql.="link text NOT NULL, icon text)";
             $conn->query($sql);
         }
         
         if($last_updated < "2019-12-01")
         {
             $sql="alter table supplier add email TEXT NULL AFTER `supplier_name`, add contact_no TEXT NULL AFTER `email`, add address TEXT NULL AFTER `contact_no`";
             $conn->query($sql);
         }
         if($last_updated < "2019-12-18")
         {
             $sql=" ALTER TABLE `shop_details` ADD `icon` TEXT NULL AFTER `logo`, ADD `technical_email` TEXT NULL AFTER `icon`, ADD `technical_contact` TEXT NULL AFTER `technical_email`, ADD `technical_alternate_contact` TEXT NULL AFTER `technical_contact`";
             $conn->query($sql);
         }
        
         
        //MENTION CONDITIONS HERE ONLY BEFORE UNSETTING THE SESSION VARIABLE
        $current_date=date('Y-m-d h:m:s');
        $sql="update user_plans set last_updated='$current_date' where user_id=(select u_id from user_profiles where domain='".$_SESSION['domain']."')";
        $conn_admin->query($sql);
